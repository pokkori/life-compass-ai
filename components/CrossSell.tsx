  ...
  return (
    <section className="px-4 py-12" aria-labelledby="cross-sell-heading">
      <div className="max-w-4xl mx-auto">
        <h2 id="cross-sell-heading" className="text-xl font-bold text-center mb-6 opacity-80">他のAIツールも試してみる</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {display.map((tool) => (
            <a
              ...
              className="block p-3 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 transition-all duration-200 text-center"
            >
              <p className="font-medium text-sm">{tool.name}</p>
              <p className="text-xs opacity-50 mt-1">{tool.desc}</p>
            </a>
          ))}
        </div>
      </div>
